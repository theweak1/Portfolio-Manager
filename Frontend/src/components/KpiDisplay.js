...
const KpiDisplay = ({ monthlyFinancials, startupId }) => {
	...
	useEffect(() => {
		const timeframeLength =
			timeframe === 'monthly' ? 1 : timeframe === 'trimester' ? 3 : 12;
		const newFinancials =
			timeframe === 'monthly'
	...
	const calculateKpis = (financialsArray) => {
		const totalRevenue = financialsArray.reduce(
			(total, month) => total + month.revenue,
			0
		);
		const totalCostOfGoodsSold = financialsArray.reduce(
			(total, month) => total + month.costOfGoodsSold,
			0
		);
		const totalAllExpenses = financialsArray.reduce(
			(total, month) => total + month.allExpenses,
			0
		);
		const totalOperatingExpenses = financialsArray.reduce(
		...
		if (financialsArray.length >= 2) {
			const currentRevenue =
				financialsArray[financialsArray.length - 1].revenue;
			const previousRevenue =
				financialsArray[financialsArray.length - 2].revenue;
			revenueGrowth = (currentRevenue / previousRevenue - 1).toFixed(2);
		} else if (financialsArray.length === 1) {
			revenueGrowth = 'N/A';
		}
		const cashBurnMonths =
			monthlyFinancials.length >= 3 ? 3 : monthlyFinancials.length;
		const lastMonthsExpenses = monthlyFinancials
			.slice(-cashBurnMonths)
			.reduce((total, month) => total + month.operatingExpenses, 0);
		const cashBurnRate = (lastMonthsExpenses / cashBurnMonths).toFixed(2); // Always calculated based on the last three months or less
		const runway =
			cashBurnRate !== '0.00'
				? (
						financialsArray[financialsArray.length - 1].cashInBank /
						cashBurnRate
				  ).toFixed(2)
				: '0';
					...
					<button
						className={`py-2 px-4 rounded ${
							timeframe === 'monthly'
								...
								: 'bg-white text-black  border-2 border-yellow'
						}`}
