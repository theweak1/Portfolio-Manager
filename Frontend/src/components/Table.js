...
function Table() {
		...
		const fetchCapTable = async () => {
			if (!auth.token) {
				...
				try {
					...
					setPreMoneyValuationSeed(
						responseData.data.captable.preMoneyValuationSeed
					);
					setSeriesRound(responseData.data.captable.seriesRound);
					setPreMoneyValuationSeriesRound(
						responseData.data.captable.preMoneyValuationSeriesRound
					);
				} catch (err) {}
			}
		};
	...
	const [preMoneyValuationSeriesRound, setPreMoneyValuationSeriesRound] =
		useState(0);
	...
	const handleInputChange = (event, rowIndex, columnIndex) => {
		...
		if (columnIndex === 2) {
			...
			newData[rowIndex][columnIndex] = value;
			newData[rowIndex][7] =
				(newData[rowIndex][4] + newData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
		} else if (columnIndex === 5) {
			newData[rowIndex][columnIndex] = value;
			newData[rowIndex][6] =
				newData[rowIndex][5] / handleSumAllShareOwnership();
			newData[rowIndex][7] =
				(newData[rowIndex][4] + newData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
	...
	const handleInputChangeMembers = (event, rowIndex, columnIndex) => {
		...
		if (columnIndex === 1) {
			...
			newMembersData[rowIndex][columnIndex] = value;
			newMembersData[rowIndex][7] =
				(newMembersData[rowIndex][4] + newMembersData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
		} else if (columnIndex === 5) {
			newMembersData[rowIndex][columnIndex] = value;
			newMembersData[rowIndex][2] = newMembersData[rowIndex][5] * sharePrice;
			newMembersData[rowIndex][6] =
				newMembersData[rowIndex][5] / handleSumAllShareOwnership();
		} else if (columnIndex === 6) {
			newMembersData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 7) {
			newMembersData[rowIndex][columnIndex] = value;
	...
	const handleInputChangePreRound = (event, rowIndex, columnIndex) => {
		...
		if (columnIndex === 2) {
			...
			newPreRoundData[rowIndex][3] =
				angelPrice !== 0
					? Math.round(newPreRoundData[rowIndex][2] / angelPrice)
					: 0;
			...
			newPreRoundData[rowIndex][columnIndex] = value;
			newPreRoundData[rowIndex][6] =
				newPreRoundData[rowIndex][5] / handleSumAllShareOwnership();
			newPreRoundData[rowIndex][7] =
				(newPreRoundData[rowIndex][4] + newPreRoundData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
		} else if (columnIndex === 6) {
			newPreRoundData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 7) {
	...
	const handleInputChangeSeedRound = (event, rowIndex, columnIndex) => {
		...
		if (columnIndex === 2) {
			...
			newSeedRoundData[rowIndex][3] =
				seedPrice !== 0
					? Math.round(newSeedRoundData[rowIndex][2] / seedPrice)
					: 0;
		} else if (columnIndex === 3) {
			newSeedRoundData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 4) {
			newSeedRoundData[rowIndex][columnIndex] = value;
			newSeedRoundData[rowIndex][7] =
				(newSeedRoundData[rowIndex][4] + newSeedRoundData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
		} else if (columnIndex === 5) {
			newSeedRoundData[rowIndex][columnIndex] = value;
			newSeedRoundData[rowIndex][6] =
				newSeedRoundData[rowIndex][5] / handleSumAllShareOwnership();
		} else if (columnIndex === 6) {
			newSeedRoundData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 7) {
			newSeedRoundData[rowIndex][columnIndex] = value;
	...
	const handleInputChangeSeriesRound = (event, rowIndex, columnIndex) => {
		...
		if (columnIndex === 2) {
			...
			newSeriesRoundData[rowIndex][3] =
				seriesPrice !== 0
					? Math.round(newSeriesRoundData[rowIndex][2] / seriesPrice)
					: 0;
		} else if (columnIndex === 3) {
			newSeriesRoundData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 4) {
			...
			newSeriesRoundData[rowIndex][columnIndex] = value;
			newSeriesRoundData[rowIndex][6] =
				newSeriesRoundData[rowIndex][5] / handleSumAllShareOwnership();
			newSeriesRoundData[rowIndex][7] =
				(newSeriesRoundData[rowIndex][4] + newSeriesRoundData[rowIndex][5]) /
				(handleSumAllOptionShares() + handleSumAllShareOwnership());
		} else if (columnIndex === 6) {
			newSeriesRoundData[rowIndex][columnIndex] = value;
		} else if (columnIndex === 7) {
	...
	const handleSharePrice = (event) => {
		const newSharePrice =
			event.target.value.trim() === '' ? 0 : parseFloat(event.target.value, 10);
	...
	const handleSumAllInvestmentAmount = () => {
		...
		sum =
			investmentAmountSum() +
			membersInvestmentAmountSum() +
			preRoundInvestmentAmountSum() +
			seedRoundInvestmentAmountSum() +
			serieseRoundInvestmentAmountSum();
		return sum;
	};
	...
	const handleSumAllShareOwnership = () => {
		...
		sum =
			sharesOwnership() +
			MemberSharesOwnership() +
			preRoundSharesOwnership() +
			seedRoundSharesOwnership() +
			seriesRoundSharesOwnership();
		return sum;
	};
	...
	const sumOfOwnershipPercent = () => {
		...
		sum =
			percentOwnership() +
			percentOwnershipMembers() +
			percentOwnershipPreRound() +
			percentOwnershipSeedRound() +
			percentOwnershipSeriesRound();
		return sum * 100;
	};
	...
	const dilutedOwnershipSum = () => {
		...
		sum =
			dilutedOwnership() +
			dilutedOwnershipMembers() +
			dilutedOwnershipPreRound() +
			dilutedOwnershipSeedRound() +
			dilutedOwnershipSeriesRound();
		return sum * 100;
	};
				...
				<div>
						...
						<table className="w-full table-fixed">
							<thead>
								<tr className="   text-white bg-darkGrey text-center">
									...
									<th>Option shares</th>
									<th>
										Issued and outstandings # shares Ownership as of {<Date />}
									</th>
									<th>% Ownership as of {<Date />}</th>
									<th>% fully diluted ownership as of {<Date />}</th>
								</tr>
								<tr>
									<th className="text-white bg-lightGrey border-black">
										Founders
									</th>
								</tr>
								<tr>
							...
							<tbody className="">
								{data.map((row, rowIndex) => (
									<tr key={rowIndex} className="border-black ">
											...
											<td key={columnIndex} className="border-black border-2">
												<input
													className={`${
														...
														columnIndex === 3 ||
														columnIndex === 4
															? 'bg-yellow bg-opacity-50 w-full'
															: ''
													}`}
													type="text"
													value={
														columnIndex === 6 || columnIndex === 7
															? `${parseFloat(cell * 100).toFixed(2)}%`
															: columnIndex === 2
															? Math.round(cell)
										...
										<td>
											<button
												className="ml-12  py-1 px-4 bg-gray-500 hover:bg-red-600 text-white font-bold rounded-lg text-base  "
												onClick={() => handleDeleteRowData(rowIndex)}
											>
												Delete
											</button>
										</td>
								...
								{membersData.map((row, rowIndex) => (
									<tr key={rowIndex} className="border-black ">
										...
										<td>
											<button
												className="ml-12  py-1 px-4 bg-gray-500 hover:bg-red-600 text-white font-bold rounded-lg text-base  "
												onClick={() => handleDeleteRowMembers(rowIndex)}
											>
												Delete
											</button>
										</td>
					...
					<div>
						<table className="w-full table-fixed">
							<thead>
								<tr>
									<th className="text-white bg-grey border-black">
										PRE-SEED Round
									</th>
									<th className="border border-black">
										<input
								...
								<tr>
									...
									<th>
										{angelRound == preRoundInvestmentAmountSum()
											? 'Check: True'
											: 'Check: False'}
									</th>
								</tr>
								...
								{preRoundData.map((row, rowIndex) => (
									<tr key={rowIndex} className="border-black ">
										...
										<td>
											<button
												className="ml-12  py-1 px-4 bg-gray-500 hover:bg-red-600 text-white font-bold rounded-lg text-base  "
												onClick={() => handleDeleteRowPreRound(rowIndex)}
											>
												Delete
											</button>
										</td>
						...
						<table className="w-full table-fixed">
							<thead>
								...
								<tr>
									...
									<th>
										{seedRound == seedRoundInvestmentAmountSum()
											? 'Check: True'
											: 'Check: False'}
									</th>
								</tr>
								...
								{seedRoundData.map((row, rowIndex) => (
									<tr key={rowIndex} className="border-black ">
										...
										<td>
											<button
												className="ml-12  py-1 px-4 bg-gray-500 hover:bg-red-600 text-white font-bold rounded-lg text-base  "
												onClick={() => handleDeleteRowSeedRound(rowIndex)}
											>
												Delete
											</button>
										</td>
					...
					<div>
						<table className="w-full table-fixed">
							<thead>
								...
								<tr>
									...
									<th>
										{seedRound == serieseRoundInvestmentAmountSum()
											? 'Check: True'
											: 'Check: False'}
									</th>
								</tr>
								...
								{seriesRoundData.map((row, rowIndex) => (
									<tr key={rowIndex} className="border-black ">
										...
										<td>
											<button
												className="ml-12  py-1 px-4 bg-gray-500 hover:bg-red-600 text-white font-bold rounded-lg text-base  "
												onClick={() => handleDeleteRowSeriesRound(rowIndex)}
											>
												Delete
											</button>
										</td>
						...
						<button
							onClick={handleAddRowSeriesRound}
							className="w-full my-5 py-2 bg-yellow shadow-lg shadow-yellow-500/50 hover:shadow-yellow-500/40 text-black font-semibold rounded-lg"
						>
							Add Row
						</button>
					...
					<div>
						<table className="w-full table-fixed">
							<thead>
								<tr>
									<th className="border-black border-2 text-white bg-grey">
										Total
									</th>
									<th className="border-black border-2"></th>
									<th className="border-black border-2 text-white bg-grey">
										{numberFormatter(handleSumAllInvestmentAmount())}
									</th>
									<th className="border-black border-2"></th>
									<th className="border-black border-2 text-white bg-grey">
										{numberFormatter(handleSumAllOptionShares())}
									</th>
									<th className="border-black border-2 text-white bg-grey">
										{numberFormatter(handleSumAllShareOwnership())}
									</th>
									<th className="border-black border-2 text-white bg-grey">
