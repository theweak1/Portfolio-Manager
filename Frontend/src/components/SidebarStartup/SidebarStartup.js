...
export default function SidebarStartup() {
...
useEffect(()=>{
        ...
        if(!auth.token){}else{
            try {
                ...
                `,"GET",null, 
                {
                  ...
                  setsbData([...SidebarDataStartup,  {
                    title: "Connect to Waves",
                    path: responseData.redirectLink,
                  }])
            } catch (err) {}
    ...
    return(
        <> 
            ...
            <nav className={sidebar ? "fixed flex justify-center w-44 sm:w-52 md:w-60 h-full z-10 top-0 left-0 transition-all duration-[250ms] ease-out  bg-white shadow-sm mt-20" 
                                    : "fixed flex justify-center w-44 sm:w-52 md:w-60 h-full z-10 top-0 -left-full transition-all duration-[250ms] ease-in bg-white shadow-sm mt-20"}>{/*nav-menu shown(false) -left-full*/}                                                                      
                    ...
                    {sbData.map((item, index) => {
                        return (
                            ...
                            <Link to={item.path} onClick={()=>setSidebar(!sidebar)}>{/*nav-text*/}
                                <span className="flex justify-start items-center p-5 mx-2 text-md sm:text-xl md:text-2xl rounded-sm text-darkGrey hover:bg-lightGrey hover:opacity-80 hover:bg-opacity-10 shadow-sm mt-20">{item.title}</span>
                            </Link>          
                            </li>
